...
- ✅ **連結鍵=`病歷號`(2026-06-24 更正)**:新檔 `EYLEA 8mg 恩慈整理完成 (3).xlsx`「重新整理過」sheet 的 `病歷號`(如 20592156/21396405)與 h5 `patient_id`(如 20242932/21394651)**同 ID 系統**(同 6–8 碼、號段交錯)→ **join=(病歷號, OS/OD eye, 施打日期↔visit date)**。⚠️**更正(2026-07-01,見 [[excel-join-keys-verified]])**:`20250811更新` 版 pool「Chart no.」(D欄)實測**多數是 7–8 位真病歷號**(7位×529/8位×258),**可用**;舊說「5 碼別序號勿用」只對前幾列舊號成立。pool `Name`(E欄)此版放**真實姓名**(未去識別化)。pilot 17 病人 ∩ EYLEA=0 只因 pilot 非 EYLEA cohort(待真實 OCT 重疊確認 patient_id==病歷號)。
