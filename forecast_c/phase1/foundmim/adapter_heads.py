...
class Interpolation(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        return interpolate(x, self.target_size, mode="bilinear")


class LinearAdapterHead(nn.Module):
    """Adapter head contains a single linear layer."""

    def __init__(self, source_size: tuple[int, ...] | torch.Size, target_size: tuple[int, ...] | torch.Size):
...
class MLPAdapterHead(nn.Module):
    ...
    def __init__(
        ...
        if self.source_size[1] != self.target_size[1]:
            print(f"Resolution Mismatch. Interpolating from {self.source_size[1:]} to {self.target_size[1:]}")
            self.interpolation = nn.Sequential(
        ...
        if num_layer == 1:
            ...
            self.adapter = nn.Sequential(
                ...
                *list(
                ...
                #     chain.from_iterable(
                #         [
                #             [nn.LayerNorm(hidden_dim), nn.GELU(), nn.Linear(hidden_dim, hidden_dim)]
