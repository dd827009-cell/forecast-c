...
from .adapter_heads import ConvAdapterHead, LightConvAdapterHead, MLPAdapterHead, LinearAdapterHead
...
class FeatureTranslator(nn.Module):
    ...
    def __init__(
        ...
        #     ),
        # )
        self.backbone_adapter = nn.Identity()  # No need to change the channel dim
    ...
    def forward(
        ...
        features = {
            t: self.translator_heads[self.legit_target_model_name_map[t]](x, backbone_no_cls=backbone_no_cls)
...
class MLPFeatureTranslator(FeatureTranslator):
    def __init__(
        ...
        target_feature_sizes: dict[str, torch.Size | tuple[int, ...]],
        translator_hidden_size: int = 1024,
        translator_n_layer: int = 4,  # originally 3; 4 is best
    ) -> None:
    ...
    def build_translator_heads(self) -> nn.ModuleDict:
        ...
        source_size = self.backbone_feature_size  # bypass backbone_adapter
    ...
    def build_translator_heads(self) -> nn.ModuleDict:
        ...
        for target_model, target_size in self.target_feature_sizes.items():
            if "_cls" in target_model:
                head = LinearAdapterHead(source_size=self.backbone_feature_size, target_size=target_size)
            else:
