# This is the main implementation of the pre-training framework in the thesis.
# --------------------------------------------------------
# References:
...
class MaskedImageModelingViT(nn.Module):
    ...
    def __init__(
        ...
        teacher_names=[],  # e.g., ["dinov2_vitl14", "dfn_clip_vitl14"]
        ...
        translator_type_teacher="lconv",
        feature_sizes_teacher=None,  # e.g., [[1024, 16, 16], [1024, 16, 16]] for two ViT-Large teachers
        adaptive_patch_weighting=False,  # adaptive teacher loss weighting
        adaptive_patch_weighting_balance_coef=1e-1,  # load-balancing hyperparameter
        # --------------------------------------------------------------------------
    ):
        super().__init__()
        ...
        # --------------------------------------------------------------------------
        # Teacher feature reconstruction specifics
        self.num_teachers = 0
        ...
        if len(teacher_names) > 0:
            ...
            if not self.direct_distillation:
                ...
                self.decoder_embed_teacher = nn.Linear(embed_dim, decoder_embed_dim_teacher, bias=True)

                self.mask_token_teacher = nn.Parameter(torch.zeros(1, 1, decoder_embed_dim_teacher))

                self.cross_attn_decoder_teacher = cross_attn_decoder_teacher
                if self.cross_attn_decoder_teacher:
                    self.decoder_blocks_teacher = nn.ModuleList(
                        [
                            CrossAttentionBlock(
                ...
                self.decoder_norm_teacher = norm_layer(decoder_embed_dim_teacher)

            # feature translator borrowed from "Theia: Distilling Diverse Vision Foundation Models for Robot Learning"
            # Reference: https://arxiv.org/abs/2407.20179
            # https://github.com/bdaiinstitute/theia/blob/main/src/theia/models/feature_translators.py
            self.feature_translators = build_feature_translator(
                translator_type=translator_type_teacher,
                backbone_feature_size=(
                    embed_dim if self.direct_distillation else decoder_embed_dim_teacher,
                    int(math.sqrt(num_patches)),
                    int(math.sqrt(num_patches)),
                ),
                target_feature_sizes={t: s for t, s in zip(teacher_names, feature_sizes_teacher)},
                translator_hidden_size=2048,
            )

            # Adaptive teacher loss weighting module and load-balancing hyperparameter
            self.adaptive_patch_weighting = adaptive_patch_weighting
            if self.adaptive_patch_weighting:
                self.adaptive_patch_weighting_proj = LightConvAdapterHead(
                    source_size=(
                        embed_dim if self.direct_distillation else decoder_embed_dim_teacher,
                        int(math.sqrt(num_patches)),
                        int(math.sqrt(num_patches)),
                    ),
                    target_size=(self.num_teachers, 16, 16),  # currently hard-coded to 16x16 teacher feature maps!
                    hidden_size_factor=1.0,
                )
                self.adaptive_patch_weighting_balance_coef = adaptive_patch_weighting_balance_coef
    ...
    def initialize_weights(self):
        ...
        if self.num_teachers > 0 and not self.direct_distillation:
            ...
            torch.nn.init.normal_(self.mask_token_teacher, std=0.02)
    ...
    def forward_decoder_teacher(self, x, ids_restore, ids_keep, ids_mask):
        if not self.direct_distillation:
            # embed tokens
            x = self.decoder_embed_teacher(x)

            # append mask tokens to sequence
            mask_tokens = self.mask_token_teacher.repeat(x.shape[0], ids_restore.shape[1] + 1 - x.shape[1], 1)
            x_ = torch.cat([x[:, 1:, :], mask_tokens], dim=1)  # no cls token
            x_ = torch.gather(x_, dim=1, index=ids_restore.unsqueeze(-1).repeat(1, 1, x.shape[2]))  # unshuffle
            x = torch.cat([x[:, :1, :], x_], dim=1)  # append cls token
            ...
            x = self.decoder_norm_teacher(x)

        # remove cls token
        x = x[:, 1:, :]

        # feature translator for all teachers
        features = self.feature_translators(x, target_model_names=self.teacher_names, backbone_no_cls=True)

        # Compute teacher loss weighting on each patch token
        if self.adaptive_patch_weighting:
            adaptive_patch_logits = self.adaptive_patch_weighting_proj(x, backbone_no_cls=True)  # [B, L, num_teachers]
            adaptive_patch_weights = torch.softmax(adaptive_patch_logits, dim=-1)  # [B, L, num_teachers]
        else:
            adaptive_patch_weights = None

        return features, adaptive_patch_weights
    ...
    def forward_loss_teacher(self, target, pred, adaptive_patch_weight=None):
        """
        target: [N, L, D]
        pred: [N, L, D]
        mask: [N, L], 0 is keep, 1 is remove,
        ...
        if adaptive_patch_weight is not None:
            loss = loss * adaptive_patch_weight  # [N, L], weighted loss per patch
    ...
    def forward(self, imgs, mask_ratio=0.75, teacher_features={}):
        ...
        latent, mask, ids_restore, ids_keep, ids_mask = self.forward_encoder(imgs, mask_ratio)
        ...
        if len(teacher_features) == 0:
            ...
            return loss, None, None, pred, mask  # no teacher

        # feature reconstruction
        # pred_teacher_features = self.forward_decoder_teacher(latent, ids_restore, ids_keep, ids_mask)
        pred_teacher_features, adaptive_patch_weights = self.forward_decoder_teacher(
            latent, ids_restore, ids_keep, ids_mask
        )
        ...
        for idx, teacher_name in enumerate(self.teacher_names):
            ...
            loss_teacher = self.forward_loss_teacher(
                ...
                adaptive_patch_weights[:, :, idx] if self.adaptive_patch_weighting else None,
            )
            loss_teacher_dict[teacher_name]["patch"] = loss_teacher * self.loss_weights_teacher[teacher_name]
            loss_teacher_dict[teacher_name]["load_balance"] = (
                adaptive_patch_weights[:, :, idx].mean().cpu().item() if self.adaptive_patch_weighting else 0.0
            )  # only for logging
            # print(f"Teacher: {teacher_name}")
            # print(adaptive_patch_weights[:, :, idx])

        if self.adaptive_patch_weighting:
            # Compute load balancing loss: encourage the per-sample teacher distribution to be uniform
            teacher_mean = adaptive_patch_weights.mean(dim=1)  # [B, num_teachers]
            loss_balance = ((teacher_mean - 1.0 / self.num_teachers) ** 2).mean()
            loss_teacher_balance = loss_balance * self.adaptive_patch_weighting_balance_coef
        else:
            loss_teacher_balance = None
